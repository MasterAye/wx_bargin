    ...
    util.request(api.GetCountryCode, {}, "POST").then(res => {
      ...
      if (res.errno == 0) {
        for (let i = 0; i < res.data.length; i++) {
          // console.log(i)
          that.data.AreacodeArray.push(
            " [ " +
              res.data[i].country_code +
              " ] " +
              res.data[i].country_name_chinese +
              "-" +
              res.data[i].country_name_english
          );
        }
        that.setData({
  ...
  getcode() {
    ...
    if (this.data.inputMobile == "") {
      ...
      util
        ...
        .then(function(res) {
          console.log("SedDomesticSode", res);
