...
Page({
  ...
  onLoad: function (options) {
    ...
    console.log("111111111111111111111111111:", options.id);
    if (options.id) {
      that.setData({
  ...
  findBargainInfo() {
    ...
    util.request(
      api.FindUserBargain, {
        ...
        wx.hideLoading();
        // 砍价订单取消激活后，之前的分享处理
        var isAbled = res.data.bargainmain_table_info.is_abled;
        // 砍价订单直接删除时，之前的分享页面处理
        var barDeleteStatus = res.data.bargainmain_table_info;
        if (isAbled == 0 || barDeleteStatus == 0) {
          wx.showModal({
            title: '提示',
            content: '此砍价已失效，跳转到首页!',
            showCancel: false,
        ...
        WxParse.wxParse(
          "goodsDetail",
          "html",
          res.data.bargainmain_table_info.activity_detail,
          that
        );
  ...
  checkIsLaunch() {
    ...
    if (
      that.data.bargainUserInfo.iscut !== 0 &&
      that.data.bargainUserInfo.iscut !== 1
    ) {
      that.setData({
  ...
  setTimeloop() {
    ...
    var loop = setInterval(function () {
      if (
        parseInt(that.data.bargainUserInfo.end_time) - new Date().getTime() <
        0
      ) {
        that.setData({
          bar_is_End: true
    ...
    util
      ...
      .then(res => {
        ...
        if (res.errno === 0) {
          ...
          if (that.data.IS_CUT_SUCCESS) {
            that.showAdressModal();
          } else {
            wx.showModal({
              title: "提示！",
              content: "立即支付后，不能继续砍价",
              success: function (res) {
                if (res.confirm) {
                  that.showAdressModal();
                }
  ...
  selectAddress(e) {
    ...
    util
      .request(
        api.AfterBargainSuccessAdressDetail, {
          id: id
        },
        "POST"
      )
            ...
            success: function (res) {
              if (res.confirm) {
                wx.showModal({
                  title: "请再次确认 ！",
                  content: "请确认商品为" +
                    that.data.bargainUserInfo.goods_name +
                    " , " +
                    that.data.bargainUserInfo.goods_sku_value +
                    " , " +
                    " 吗 ？ 点击确认支付 " +
                    that.data.bargainUserInfo.goods_lowest_price +
                    " 元购买 ！ ",
                  success: function (res) {
                    if (res.confirm) {
                      wx.showLoading({
                      ...
                      util
                        .request(
                          api.AfterBargainSuccessSetOrder, {
                            selectAdressId: id,
  ...
  showShareModal: function () {
    ...
    this.animation = animation;
    animation.translateY(250).step();
    if (that.data.IS_CUT_SUCCESS) { } else {
      this.setData({
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      //title: api.slogan_share,
      title: self.data.user_IN_Info.nickname + "邀您参与" + self.data.bargainUserInfo.goods_name + "砍价活动",
      path: "/pages/Resagin_bargain_goods/Resagin_bargain_goods?id=" + self.data.bargain_user_table_id,
      imageUrl: self.data.bargainUserInfo.goods_pic
    };
  }
